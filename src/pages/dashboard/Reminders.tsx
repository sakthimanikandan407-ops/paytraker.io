        ...
        <div className="space-y-12 pb-20">
            ...
            <div className="flex gap-12 border-b border-white/5">
                {[
                    ...
                    { id: 'log', label: 'Email History', icon: History },
                    { id: 'scheduled', label: 'Scheduled Reminders', icon: Calendar },
                ].map(tab => (
                    <button
                        key={tab.id}
                ...
                <div className="space-y-16 animate-in fade-in duration-700">
                                ...
                                return (
                                    <div key={rule.id} className="flex flex-col items-center group relative">
                                        ...
                                        <div className="mt-6 text-center">
                                            <div className={`text-[10px] font-black uppercase tracking-[0.2em] mb-2 px-3 py-1 rounded-full border ${
                                                rule.days_offset < 0 ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' :
                                                rule.days_offset === 0 ? 'text-indigo-400 border-indigo-500/20 bg-indigo-500/5' :
                                                rule.days_offset <= 3 ? 'text-amber-400 border-amber-500/20 bg-amber-500/5' :
                                                'text-rose-400 border-rose-500/20 bg-rose-500/5'
                                            }`}>
                                                {rule.days_offset < 0 ? 'Friendly' :
                                                 rule.days_offset === 0 ? 'Professional' :
                                                 rule.days_offset <= 3 ? 'Firm' : 'Final'}
                                            </div>
                                            <p className={`text-xs font-black uppercase tracking-widest transition-colors ${rule.is_active ? 'text-white' : 'text-slate-600'}`}>
                                                {Math.abs(rule.days_offset)} days {rule.trigger_type.replace('_', ' ')}
                                            </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="bg-indigo-600/5 p-10 rounded-[2.5rem] border border-indigo-500/10 shadow-2xl relative overflow-hidden group">
                            ...
                            <h3 className="text-xl font-black text-white mb-3 uppercase tracking-tight">How reminders work</h3>
                            <p className="text-slate-400 text-sm mb-10 leading-relaxed font-medium">
                                PayTrack's autonomous engine monitors invoices on a micro-cycle. Based on active rules,
                                friendly notifications are dispatched to clients to ensure you get paid on time.
                            </p>
                        ...
                        <div className="bg-white/5 backdrop-blur-3xl p-10 rounded-[2.5rem] border border-white/10 shadow-2xl flex flex-col justify-center">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-white uppercase tracking-tight text-lg">Browser Notifications</h4>
                                    <p className="text-sm font-medium text-slate-400 mt-1">Get alerts when reminders are sent or payments are received.</p>
                                </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-white/5 text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] border-b border-white/5">
                                    ...
                                    <th className="px-8 py-6">Reminder Status</th>
                                    <th className="px-8 py-6">Sent At</th>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in duration-700">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden group hover:border-indigo-500/30 transition-all">
                            ...
                            <h4 className="font-black text-white mb-2 uppercase tracking-tight text-lg">Next Reminder</h4>
                            <p className="text-sm font-medium text-slate-500 mb-8 leading-relaxed">Reminder scheduled to be sent automatically.</p>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-2xl z-50 flex items-center justify-center p-6">
                    <div className="bg-slate-900 border border-white/10 rounded-[3rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                        <div className="p-12">
                            <h3 className="text-2xl font-black text-white mb-2 uppercase tracking-tight">New Reminder Rule</h3>
                            <p className="text-sm font-medium text-slate-500 mb-10 uppercase tracking-widest">Set up automatic email reminders</p>
                            ...
                            <form onSubmit={async (e: any) => {
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        />
                                        <span className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Days</span>
                                    </div>
                                </div>
