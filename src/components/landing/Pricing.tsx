  ...
  return (
    <section id="pricing" className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            return (
              ...
              >
                <div className={`flex-grow bg-slate-950 rounded-[2.45rem] p-8 border border-white/5 transition-all ${plan.highlight ? 'bg-slate-950/90 shadow-inner' : 'hover:bg-slate-900/50 hover:border-white/10'}`}>
                  ...
                  <a
                    href={isFree ? (user ? '/dashboard' : '/signup') : (user ? `/dashboard/payment?plan=${plan.name.toLowerCase()}` : `/signup?plan=${plan.name.toLowerCase()}`)}
        ...
        <div className="text-center mt-20 text-slate-500 text-xs font-bold flex items-center justify-center gap-3">
          <div className="w-10 h-px bg-white/5" />
          Powered by Industry-Standard Encryption & Dodo Payments
          <div className="w-10 h-px bg-white/5" />
        </div>
      </div>
    </section>
